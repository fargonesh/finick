...
fn kind_icon(r: &index::ty::SearchResult) -> &'static str {
    if r.is_desktop { APPS } else if r.is_dir { FOLDER } else if r.is_executable { TERMINAL } else { FILE }
}

fn kind_label(r: &index::ty::SearchResult) -> &'static str {
    if r.is_desktop { "App" } else if r.is_dir { "Folder" } else if r.is_executable { "Exec" } else { "File" }
}
            ...
            move |e: Event<KeyboardEventData>| {
                ...
                match e.data().key.clone() {
                    ...
                    Key::Named(NamedKey::ArrowDown) => {
                        if len > 0 { sel_state.set((cur + 1) % len) }
                    }
                    Key::Named(NamedKey::ArrowUp) => {
                        if len > 0 { sel_state.set(if cur == 0 { len - 1 } else { cur - 1 }) }
                    }
                        ...
                        .child(if results_len == 0 {
                            rect()
                                ...
                                .child(
                                    rect()
                                        ...
                                        .child(label().font_size(13.).color(t.text_muted).text(if query.read().trim().is_empty() { "Type to search apps & files" } else { "No results — press Enter to run as command" })),
                                )
                                .into_element()
                        } else {
                            ScrollView::new().width(Size::fill()).height(Size::fill()).child(
                                rect()
                                    ...
                                    .vertical()
                                    .spacing(4.)
                                    .children(results.read().iter().enumerate().map(|(idx, item)| {
                                        let is_sel = idx == sel;
                                        ...
                                        rect()
                                            ...
                                            .child(
                                                rect()
                                                    ...
                                                    .border(Border::new().width(1.).fill(t.border))
                                                    .center()
                                                    .child(icon(kind_icon(&item_for_icon), 18., if is_sel { t.accent } else { t.text_dim })),
                                            )
                                            .child(
                                                rect()
                                                    .width(Size::fill())
                                                    .vertical()
                                                    .content(Content::Flex)
                                                    .child(label().font_size(13.).font_weight(FontWeight::SEMI_BOLD).color(t.text).text(item_for_label.name.clone()))
                                                    .child(label().font_size(11.).color(t.text_dim).text(item_for_label.path.clone())),
                                            )
                                            .child(
                                                rect()
                                                    .padding((4., 8.))
                                                    ...
                                                    .child(label().font_size(10.).font_weight(FontWeight::BOLD).color(if is_sel { t.panel } else { t.text_muted }).text(kind_label(item).to_string())),
                                            )
                                            .into_element()
                                    }))
                            )
