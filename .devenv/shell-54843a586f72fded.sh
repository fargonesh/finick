...
prefix='/nix/store/zpa41m9hz1834fi5wpr3x2y569bx94mm-devenv-shell-env'
...
for var in TMP TMPDIR TEMP TEMPDIR; do
  if [ -n "${!var-}" ] && [ "${!var}" = "${NIX_BUILD_TOP-}" ]; then
    export "$var"=/tmp/nix-shell.0d3dzY
  fi
done
...
NIX_STORE='/nix/store'
export NIX_STORE
out='/nix/store/zpa41m9hz1834fi5wpr3x2y569bx94mm-devenv-shell-env'
export out
...
exec /tmp/nix-shell.0d3dzY/devenv-envkCRsZL/script 